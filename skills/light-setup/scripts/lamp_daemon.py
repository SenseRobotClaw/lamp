...
环境变量（可覆盖）：
  LIGHTWING_WORKDIR    工作目录（默认: ~/.openclaw/workspace）
  LIGHTWING_SESSION    session.json 路径（默认: $WORKDIR/.lightwing/session.json）
  LIGHTWING_LOG        日志文件（默认: /tmp/lamp_daemon.log）
  LIGHTWING_FEISHU_TARGET  飞书推送目标用户ID（默认: ou_bf4314b08df70c2b95f2c2b33ac1f4ce）
"""
...
# ── 路径自动检测 ──────────────────────────────────────────────
def find_path():
    """自动检测 WORKDIR / OPENCLAW / SESSION_FILE，无需用户配置"""
    home = os.path.expanduser("~")

    # WORKDIR
    workdir = os.environ.get("LIGHTWING_WORKDIR", "")
    ...
    # OPENCLAW
    openclaw = os.environ.get("LIGHTWING_OPENCLAW", "")
    ...
    # SESSION_FILE
    if os.environ.get("LIGHTWING_SESSION"):
        sess_path = os.environ["LIGHTWING_SESSION"]
    else:
        sess_path = os.path.join(workdir, ".lightwing", "session.json")

    return workdir, sess_path, openclaw

WORKDIR, SESSION_FILE, OPENCLAW = find_path()

STATE_FILE     = "/tmp/lamp_last_state.json"
CMD_FILE       = "/tmp/lamp_cmd_queue.txt"
LOG_FILE       = os.environ.get("LIGHTWING_LOG", "/tmp/lamp_daemon.log")
FEISHU_TARGET  = os.environ.get("LIGHTWING_FEISHU_TARGET", "")
